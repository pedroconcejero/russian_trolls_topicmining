...
feartrolls <- trollstot[trollstot$account_category == "Fearmonger" 
                         & trollstot$retweet == "0"
                         & trollstot$language == "English",]
dim(feartrolls)

# Creamos un corpus con el contenido de los tuits
trolls <- corpus(feartrolls$content)
...
quant_dfm <- dfm_trim(myStemMat, min_termfreq = 5)
...
png(file="FEARTROLLS_hi_res_def_def.png",
    width=3600,
    height=3000,
    res = 300,
    bg = "black")
...
save.image(file = "wkspace_FEARtrolls.rda")
